...
public class PostgreSqlAdministrativeDivisionTypeRepository implements AdministrativeDivisionTypeRepository {
    ...
    private static final String REPOSITORY = "POSTGRESQL ADMINISTRATIVE DIVISION TYPE REPOSITORY";
    ...
    public Uni<AdministrativeDivisionType> findById(final DivisionTypeId id) {
        log.info(LogUtil.log(
                REPOSITORY,
                "Start finding administrative division type by id: id=%s",
                id.value()));
    ...
    public Uni<AdministrativeDivisionType> findByCountryIdAndCode(final CountryId countryId, final String code) {
        log.info(LogUtil.log(
                REPOSITORY,
                "Start finding administrative division type by code: countryId=%s, code=%s",
                countryId.value(),
                code));
        return logFailure(pool.preparedQuery(FIND_BY_COUNTRY_AND_CODE)
                .execute(Tuple.of(countryId.value(), code))
                .onItem().transform(rows -> rowSetMapper.firstOrNull(
                        rows,
                        rowMapper::toAdministrativeDivisionType
                )), "findByCountryIdAndCode");
    }
    ...
    public Uni<List<AdministrativeDivisionType>> findByCountryId(final CountryId countryId) {
        log.info(LogUtil.log(
                REPOSITORY,
                "Start finding administrative division types: countryId=%s",
                countryId.value()));
    ...
    public Uni<List<AdministrativeDivisionType>> findByCountryIdAndStatus(
        ...
        log.info(LogUtil.log(
                REPOSITORY,
                "Start finding administrative division types by status: countryId=%s, status=%s",
                countryId.value(),
                status));
        return logFailure(pool.preparedQuery(FIND_BY_COUNTRY_AND_STATUS)
                .execute(Tuple.of(countryId.value(), status.name()))
                .onItem().transform(rows -> rowSetMapper.toList(
                        rows,
                        rowMapper::toAdministrativeDivisionType
                )), "findByCountryIdAndStatus");
    }
